...
const TEST_CONFIGURATIONS = {
  ...
  language: daplaJsUtilities.LANGUAGE.LANGUAGES.NORWEGIAN.languageCode,
  otherLanguage: daplaJsUtilities.LANGUAGE.LANGUAGES.ENGLISH.languageCode
};

const ApiContext = /*#__PURE__*/React__default['default'].createContext({
  graphqlApi: `${process.env.REACT_APP_API}${API.GRAPHQL}`,
  restApi: process.env.REACT_APP_API
});
const LanguageContext = /*#__PURE__*/React__default['default'].createContext(daplaJsUtilities.LANGUAGE.LANGUAGES.NORWEGIAN.languageCode);
...
const datasetsFromVariable = (result, type) => {
  ...
  if (datasets !== undefined) {
    const filteredDatasets = datasets.filter(entry => daplaJsUtilities.getNestedObject(entry, MODEL.GET_DATASETS).length !== 0);
